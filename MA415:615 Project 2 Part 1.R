...
ggplot(Noon_Data) + 
  ...
  labs(x = "Daily Noon Air Temp (Celcius)", y = "Daily Noon Sea Temp (Celcius)")
...
# Because the p-value p-value < 2.2e-16 < 0.01, we reject the null hypothesis that the means of 
# WTMP in 1988 and in 2017 are equal and conclude that sea temp indeed changed over 30 years.
# So my sampling doesn't affect my evaluation of sea temperature change.
# means in two years: 4.608660  6.156853 

# The reason why choosing daily noon data and whole hourly data gave us same result of "changing mean"
# is that getting one noon data every day has already created a big dataset for a year (365 sample)
# and the each temperature recorded is correlacted with the temperaature recorded last time and next time
# This is the so-called time series. 
